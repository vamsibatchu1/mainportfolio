...
export function MainNav({ className = "", onTabChange, activeTab, tabs }: MainNavProps) {
  ...
  const handleNavClick = (path: string, title: string) => {
    ...
    // If onTabChange is provided, use tab switching instead of routing
    if (onTabChange) {
  ...
  const isActive = (path: string, title: string) => {
    if (activeTab) {
      // For tab switching mode, check if the tab title matches
      return activeTab === title.toLowerCase();
    }
        ...
        <div className="content-stretch flex gap-[24px] items-center justify-start relative shrink-0">
          <div className="content-center flex flex-wrap gap-[20px] items-center justify-start relative shrink-0">
            {navItems.map((item, index) => {
              const active = isActive(item.path, item.title);
